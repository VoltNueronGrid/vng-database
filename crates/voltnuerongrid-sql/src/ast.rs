...
pub struct SelectStatement {
    ...
    /// True when the query uses RIGHT ANTI JOIN (S3-WS1-82).
    pub has_right_anti_join: bool,
    /// True when the query uses FULL SEMI JOIN (S3-WS1-83).
    pub has_full_semi_join: bool,
    /// True when the query uses FULL ANTI JOIN (S3-WS1-84).
    pub has_full_anti_join: bool,
}
    ...
    match tokens.first() {
        Some(Token::Keyword(k)) => match k.to_ascii_uppercase().as_str() {
            "SELECT" => {
                ...
                if has_right_anti_join(&up) {
                    stmt.has_right_anti_join = true;
                }
                if has_full_semi_join(&up) {
                    stmt.has_full_semi_join = true;
                }
                if has_full_anti_join(&up) {
                    stmt.has_full_anti_join = true;
                }
...
fn has_order_by_random(up: &str) -> bool {
    if let Some(idx) = up.find("ORDER BY") {
        ...
        return tail_upper.contains("RANDOM()")
            || tail_upper.contains("RAND()")
            || (tail_upper.contains("RANDOM(") && tail_upper.contains(")"));
    }
    false
}
...
fn has_outer_apply(up: &str) -> bool {
...
fn has_left_semi_join(up: &str) -> bool {
...
fn has_right_anti_join(up: &str) -> bool {
    up.contains(" RIGHT ANTI JOIN ")
}

fn has_full_semi_join(up: &str) -> bool {
    up.contains(" FULL SEMI JOIN ")
}

fn has_full_anti_join(up: &str) -> bool {
...
// ─── S3-WS1-56: has_order_by_random tests ───────────────────────────────────

#[cfg(test)]
mod order_by_random_tests {
    use super::*;

    #[test]
    fn select_order_by_random_sets_has_order_by_random() {
        let stmt = parse_one("SELECT id FROM users ORDER BY RANDOM()").unwrap();
        let Statement::Select(s) = stmt else { panic!("expected Select") };
        assert!(
            s.has_order_by_random,
            "ORDER BY RANDOM() must set has_order_by_random = true"
        );
    }

    #[test]
    fn with_cte_order_by_random_sets_has_order_by_random() {
        let stmt = parse_one("WITH t AS (SELECT id FROM users) SELECT id FROM t ORDER BY RANDOM()").unwrap();
        let Statement::Select(s) = stmt else { panic!("expected Select") };
        assert!(
            s.has_order_by_random,
            "WITH query ORDER BY RANDOM() must set has_order_by_random = true"
        );
    }

    #[test]
    fn select_order_by_named_column_keeps_has_order_by_random_false() {
...
mod anti_join_tests {
    ...
    #[test]
    fn select_anti_join_sets_has_anti_join() {
        let stmt = parse_one(
        ...
        assert!(
            s.has_anti_join,
            "SELECT ... ANTI JOIN must set has_anti_join = true"
        );
    }

    #[test]
    fn select_semi_join_keeps_has_anti_join_false() {
        let stmt = parse_one(
            "SELECT u.id FROM users u SEMI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        ...
        assert!(
            !s.has_anti_join,
            "SEMI JOIN must keep has_anti_join = false"
        );
    }

    #[test]
    fn select_without_anti_join_keeps_has_anti_join_false() {
        let stmt = parse_one("SELECT id FROM users ORDER BY id").unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        };
...
// ─── S3-WS1-77: has_outer_apply tests ─────────────────────────────────────

#[cfg(test)]
mod outer_apply_tests {
    use super::*;

    #[test]
    fn select_outer_apply_sets_has_outer_apply() {
        let stmt = parse_one(
            "SELECT u.id FROM users u OUTER APPLY (SELECT 1) x",
        )
        .unwrap();
        ...
        assert!(
            s.has_outer_apply,
            "SELECT ... OUTER APPLY must set has_outer_apply = true"
        );
    }

    #[test]
    fn select_cross_apply_keeps_has_outer_apply_false() {
        let stmt = parse_one(
            "SELECT u.id FROM users u CROSS APPLY (SELECT 1) x",
        )
        .unwrap();
        ...
        assert!(
            !s.has_outer_apply,
            "CROSS APPLY must keep has_outer_apply = false"
        );
    }

    #[test]
    fn select_without_outer_apply_keeps_has_outer_apply_false() {
        let stmt = parse_one("SELECT id FROM users ORDER BY id").unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        };
...
// ─── S3-WS1-79: has_left_semi_join tests ───────────────────────────────────

#[cfg(test)]
mod left_semi_join_tests {
    use super::*;

    #[test]
    fn select_left_semi_join_sets_has_left_semi_join() {
        let stmt = parse_one(
            "SELECT u.id FROM users u LEFT SEMI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        ...
        assert!(
            ...
            "SELECT ... LEFT SEMI JOIN must set has_left_semi_join = true"
        );
    }

    #[test]
    fn select_semi_join_keeps_has_left_semi_join_false() {
        let stmt = parse_one(
            "SELECT u.id FROM users u SEMI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        ...
        assert!(
            !s.has_left_semi_join,
            "SEMI JOIN must keep has_left_semi_join = false"
        );
    }

    #[test]
    fn select_without_left_semi_join_keeps_has_left_semi_join_false() {
        let stmt = parse_one("SELECT id FROM users ORDER BY id").unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        };
...
// ─── S3-WS1-82: has_right_anti_join tests ──────────────────────────────────

#[cfg(test)]
mod right_anti_join_tests {
    use super::*;

    #[test]
    fn select_right_anti_join_sets_has_right_anti_join() {
        let stmt = parse_one(
            "SELECT u.id FROM users u RIGHT ANTI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        ...
        assert!(
            s.has_right_anti_join,
            "SELECT ... RIGHT ANTI JOIN must set has_right_anti_join = true"
        );
    }

    #[test]
    fn select_anti_join_keeps_has_right_anti_join_false() {
        let stmt = parse_one(
            "SELECT u.id FROM users u ANTI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        ...
        assert!(
            !s.has_right_anti_join,
            "ANTI JOIN must keep has_right_anti_join = false"
        );
    }

    #[test]
    fn select_without_right_anti_join_keeps_has_right_anti_join_false() {
        let stmt = parse_one("SELECT id FROM users ORDER BY id").unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        };
        assert!(
            !s.has_right_anti_join,
            "SELECT without RIGHT ANTI JOIN must keep has_right_anti_join = false"
        );
    }
}

// ─── S3-WS1-83: has_full_semi_join tests ───────────────────────────────────

#[cfg(test)]
mod full_semi_join_tests {
    use super::*;

    #[test]
    fn select_full_semi_join_sets_has_full_semi_join() {
        let stmt = parse_one(
            "SELECT u.id FROM users u FULL SEMI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        ...
        assert!(
            s.has_full_semi_join,
            "SELECT ... FULL SEMI JOIN must set has_full_semi_join = true"
        );
    }

    #[test]
    fn select_semi_join_keeps_has_full_semi_join_false() {
        let stmt = parse_one(
            "SELECT u.id FROM users u SEMI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        ...
        assert!(
            !s.has_full_semi_join,
            "SEMI JOIN must keep has_full_semi_join = false"
        );
    }

    #[test]
    fn select_without_full_semi_join_keeps_has_full_semi_join_false() {
        let stmt = parse_one("SELECT id FROM users ORDER BY id").unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        };
        assert!(
            !s.has_full_semi_join,
            "SELECT without FULL SEMI JOIN must keep has_full_semi_join = false"
        );
    }
}

// ─── S3-WS1-84: has_full_anti_join tests ───────────────────────────────────

#[cfg(test)]
mod full_anti_join_tests {
    use super::*;

    #[test]
    fn select_full_anti_join_sets_has_full_anti_join() {
        let stmt = parse_one(
            "SELECT u.id FROM users u FULL ANTI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        ...
        assert!(
            ...
            "SELECT ... FULL ANTI JOIN must set has_full_anti_join = true"
        );
    }

    #[test]
    fn select_anti_join_keeps_has_full_anti_join_false() {
        let stmt = parse_one(
            "SELECT u.id FROM users u ANTI JOIN orders o ON o.user_id = u.id",
        )
        .unwrap();
        ...
        assert!(
            !s.has_full_anti_join,
            "ANTI JOIN must keep has_full_anti_join = false"
        );
    }

    #[test]
    fn select_without_full_anti_join_keeps_has_full_anti_join_false() {
        let stmt = parse_one("SELECT id FROM users ORDER BY id").unwrap();
        let Statement::Select(s) = stmt else {
            panic!("expected Select")
        };
