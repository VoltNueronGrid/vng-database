...
| REQ-06 | CSV/Parquet/JSON/Excel + enterprise source ingest | Epic 4, Epic 4A, Epic 7 | In Progress | WS4/WS4A ingest and streaming scaffolds with gate summaries (`tests/kpi/results/ws4/ws4-gate-summary.json`, `tests/kpi/results/ws4a/ws4a-gate-summary.json`); CSV connector (`CsvConnector` with header-aware parsing) and JSON/NDJSON connector (`JsonConnector` with key-field extraction) in `voltnuerongrid-ingest` crate (`crates/voltnuerongrid-ingest/src/csv.rs`, `crates/voltnuerongrid-ingest/src/json.rs`); runtime endpoints (`/api/v1/ingest/csv`, `/api/v1/ingest/json`, `/api/v1/ingest/status`) with unit evidence (`cargo test -p voltnuerongridd ws4_`) and smoke evidence (`tests/kpi/results/ws4/ws4-ingest-parser-smoke.json`) |
...
| REQ-11 | Indexes + constraints | Epic 2, Epic 15 | In Progress | B-tree index engine (`IndexManager`, `BTreeIndex` with create/drop/lookup/range_scan + unique enforcement) and constraint validator (`ConstraintManager` with PrimaryKey/Unique/NotNull/ForeignKey validation + committed-value tracking) in `voltnuerongrid-store` crate (`crates/voltnuerongrid-store/src/index.rs`, `crates/voltnuerongrid-store/src/constraints.rs`); runtime endpoints (`/api/v1/store/indexes/*`, `/api/v1/store/constraints/*`) with unit evidence (`cargo test -p voltnuerongridd ws2_`) and smoke/gate evidence (`tests/kpi/results/ws2/ws2-index-constraint-smoke.json`, `tests/kpi/results/ws2/ws2-gate-summary.json`) |
...
| REQ-13 | Multi-user roles and privileges | Epic 5 | In Progress | Shared RBAC privilege matrix now maps operator roles to resource-scoped grants/actions in `voltnuerongrid-auth`, and `voltnuerongridd` enforces those grants across failover/SRE/audit/autonomous control-plane handlers; validated by updated WS5 smoke evidence (`tests/kpi/results/ws5/operator-auth-smoke.json`) |
...
|---|---|---|
| WS2 Index + Constraint Smoke | Epic 2 + REQ-11 (B-tree index engine + constraint validator with PK/Unique/NotNull enforcement + runtime endpoints) | `tests/kpi/results/ws2/ws2-index-constraint-smoke.json` |
...
| Gate | Scope | Local Artifact |
|---|---|---|
| WS4 Ingest Parser Smoke | Epic 4 + REQ-06 (CSV + JSON/NDJSON connectors implementing IngestionConnector trait + runtime endpoints) | `tests/kpi/results/ws4/ws4-ingest-parser-smoke.json` |
...
|---|---|---:|---|---|---|---|---|---|
| H-01 | In Progress | 92% | improving | P0 | R2 | Added operator auth gate (`VNG_ADMIN_API_KEY` + `x-vng-admin-key`) for autonomous control endpoints, then layered registered operator identity + role binding enforcement via `x-vng-operator-id` and a resource-scoped RBAC privilege matrix for failover/SRE/audit/autonomous handlers; runtime tests and WS5 smoke harness `tests/kpi/scripts/run-ws5-operator-auth-smoke.ps1` now validate key + identity + privilege behavior in addition to prior DR policy persistence hardening | Full user/tenant role hierarchy and data-plane privileges still pending | Extend RBAC from operator roles into broader tenant/user privilege sets for REQ-13 |
