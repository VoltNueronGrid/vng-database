...
|---|---|---|---|---|
| REQ-01 | ANSI SQL + AI chat/extract/ingest/export | Epic 1, Epic 8 | In Progress | SQL analyzer baseline in `crates/voltnuerongrid-sql`; `/api/v1/sql/analyze`, `/api/v1/sql/route`, `/api/v1/sql/execute`, and `/api/v1/sql/transaction` now enforce tenant/user SQL runtime RBAC via `x-vng-tenant-id` + `x-vng-user-id` while preserving operator/admin access, validated by refreshed runtime analyze/UDF smoke artifacts (`tests/kpi/results/20260305-ws1/sql-analyze-smoke.json`, `tests/kpi/results/ws1/sql-execute-udf-smoke.json`) plus focused tenant SQL route/transaction tests and WS1 gate orchestrator evidence (`tests/kpi/results/ws1/ws1-gate-summary.json`) |
...
| REQ-11 | Indexes + constraints | Epic 2, Epic 15 | In Progress | B-tree index engine (`IndexManager`, `BTreeIndex` with create/drop/lookup/range_scan + unique enforcement) and constraint validator (`ConstraintManager` with PrimaryKey/Unique/NotNull/ForeignKey validation + committed-value tracking) in `voltnuerongrid-store` crate (`crates/voltnuerongrid-store/src/index.rs`, `crates/voltnuerongrid-store/src/constraints.rs`); runtime endpoints (`/api/v1/store/indexes/*`, `/api/v1/store/constraints/*`) now enforce operator auth + resource-scoped RBAC with unit evidence (`cargo test -p voltnuerongridd ws2_`, `cargo test -p voltnuerongridd operator_auth`) and smoke/gate evidence (`tests/kpi/results/ws2/ws2-index-constraint-smoke.json`, `tests/kpi/results/ws2/ws2-gate-summary.json`) |
...
| REQ-13 | Multi-user roles and privileges | Epic 5 | In Progress | Shared RBAC privilege matrix now maps operator and tenant-user roles to resource-scoped grants/actions in `voltnuerongrid-auth`, and `voltnuerongridd` enforces those grants across failover/SRE/audit/autonomous control-plane handlers, storage and mixed operator-or-tenant ingest surfaces, and tenant-scoped SQL analyze/route/execute/transaction runtime paths; validated by updated WS5 smoke evidence, focused `operator_auth`/`sql_runtime`/ingest tenant tests, refreshed WS1/WS2/WS4 smoke artifacts, and refreshed WS10 driver smoke (`tests/kpi/results/ws5/operator-auth-smoke.json`, `tests/kpi/results/20260305-ws1/sql-analyze-smoke.json`, `tests/kpi/results/ws1/sql-execute-udf-smoke.json`, `tests/kpi/results/ws2/ws2-index-constraint-smoke.json`, `tests/kpi/results/ws4/ws4-ingest-parser-smoke.json`, `tests/kpi/results/ws10/driver-smoke.json`) |
...
| WS2 | Epic 2 | Durability/storage/index/constraints | Storage Team | In Progress | WS0 (durability bootstrap + checkpoint/restart + disk-backed WAL adapter + WAL recovery wiring merged; index/constraint runtime endpoints now also enforce operator auth + resource-scoped RBAC as validated by `tests/kpi/results/ws2/ws2-index-constraint-smoke.json`; gate orchestrator `run-ws2-gate.ps1` -> `tests/kpi/results/ws2/ws2-gate-summary.json`; workflow wiring in `.github/workflows/ci.yml`) |
...
| WS4 | Epic 4 | High-speed ingestion pipeline | Ingestion Team | In Progress | WS2 (ingestion connector/registry scaffold + ingest runtime endpoints now enforce mixed operator-or-tenant RBAC with tenant-scoped connector storage/status visibility as validated by `tests/kpi/results/ws4/ws4-ingest-parser-smoke.json`; gate orchestrator `run-ws4-gate.ps1` -> `tests/kpi/results/ws4/ws4-gate-summary.json`; workflow wiring in `.github/workflows/ci.yml`) |
...
|---|---|---|
| WS2 Index + Constraint Smoke | Epic 2 + REQ-11 (B-tree index engine + constraint validator with PK/Unique/NotNull enforcement + runtime endpoints protected by operator auth + RBAC) | `tests/kpi/results/ws2/ws2-index-constraint-smoke.json` |
