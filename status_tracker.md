...
| REQ-06 | CSV/Parquet/JSON/Excel + enterprise source ingest | Epic 4, Epic 4A, Epic 7 | In Progress | WS4/WS4A ingest and streaming scaffolds with gate summaries (`tests/kpi/results/ws4/ws4-gate-summary.json`, `tests/kpi/results/ws4a/ws4a-gate-summary.json`); CSV connector (`CsvConnector` with header-aware parsing) and JSON/NDJSON connector (`JsonConnector` with key-field extraction) in `voltnuerongrid-ingest` crate (`crates/voltnuerongrid-ingest/src/csv.rs`, `crates/voltnuerongrid-ingest/src/json.rs`); runtime endpoints (`/api/v1/ingest/csv`, `/api/v1/ingest/json`, `/api/v1/ingest/status`) now enforce mixed operator-or-tenant RBAC, with tenant-scoped connector storage/status visibility and unit evidence (`cargo test -p voltnuerongridd ws4_`, `cargo test -p voltnuerongridd ingest_`, `cargo test -p voltnuerongridd operator_auth`) and smoke evidence (`tests/kpi/results/ws4/ws4-ingest-parser-smoke.json`) |
...
| WS4 | Epic 4 | High-speed ingestion pipeline | Ingestion Team | In Progress | WS2 (ingestion connector/registry scaffold + ingest runtime endpoints now enforce mixed operator-or-tenant RBAC with tenant-scoped connector storage/status visibility, and tenant-originated CSV/JSON runtime operations now append tenant-tagged audit records validated by focused ingest audit tests and WS5 live tenant audit smoke `tests/kpi/results/ws5/tenant-audit-runtime-smoke.json`; baseline ingest smoke remains `tests/kpi/results/ws4/ws4-ingest-parser-smoke.json`; gate orchestrator `run-ws4-gate.ps1` -> `tests/kpi/results/ws4/ws4-gate-summary.json`; workflow wiring in `.github/workflows/ci.yml`) |
...
| Gate | Scope | Local Artifact |
|---|---|---|
| WS4 Ingest Parser Smoke | Epic 4 + REQ-06 (CSV + JSON/NDJSON connectors implementing IngestionConnector trait + runtime endpoints protected by mixed operator-or-tenant RBAC) | `tests/kpi/results/ws4/ws4-ingest-parser-smoke.json` |
| WS4 Gate Summary (includes parser pack) | Epic 4 (ingest plugin scaffold + CSV/JSON parsers) | `tests/kpi/results/ws4/ws4-gate-summary.json` |
...
**Not-started requirements (6 total):** REQ-07 (multithreaded import), REQ-08 (local+cloud SaaS), REQ-10 (trillion-row scale), REQ-19 (blazing ingest/update/read), REQ-21 (any-number-user concurrency), REQ-27 (native cache engine).
