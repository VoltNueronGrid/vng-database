...
| WS12 | Epic 12 | Reliability/SRE/DR automation | SRE Team | In Progress | WS6 (runtime SRE hardening contracts: `/api/v1/sre/reliability/status`, `/api/v1/sre/rate-limit/check`, `/api/v1/sre/failure-budget/alerts`, `/api/v1/sre/dr/hooks/{policy,retry-plan,schedule,trigger,status}`, `/api/v1/sre/failure/signal`, `/api/v1/sre/gate/evaluate`; includes file-backed DR policy/runtime persistence, scheduler queue scaffold, multi-node failure signal ingestion, and expanded WS12 gate criteria + smoke harness) |
