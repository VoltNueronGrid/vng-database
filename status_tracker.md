...
| REQ-07 | Multithreaded high-speed import | Epic 4 | In Progress | **Scaffold (2026-04-04):** `IngestParallelConfig` in `crates/voltnuerongrid-ingest/src/batch_config.rs` (defaults + validated()). **Chunked loader (2026-04-04):** `load_records_chunked()` in new `crates/voltnuerongrid-ingest/src/chunked_loader.rs`; splits payload into `chunk_target_rows`-sized slices, dispatches up to `max_in_flight_tasks` chunks per wave, returns `ChunkedIngestStats` with per-chunk outcomes; 5 unit tests pass; 4 KPI tests (`ws4_chunked_load_*`) in `services/voltnuerongridd/src/main.rs` pass. **ChunkedLoader struct (2026-04-07):** stateful `ChunkedLoader` struct added with `push_chunk()` + `finalize()` API; WS4 chunked-loader smoke script `tests/kpi/scripts/run-ws4-chunked-loader-smoke.ps1` added (20/20 checks passing); WS4 gate updated to include `ws4-chunked-loader` pack. **Next:** async parallelism (Tokio task fan-out), throughput benchmarks, end-to-end HTTP endpoint integration. |
...
| REQ-23 | ACID transactions | Epic 1, Epic 2, Epic 3 | In Progress | Transaction endpoint now classifies and validates statements before commit path. **ACID state machine (2026-04-07):** `AcidTransactionRegistry` in `main.rs` (in-memory HashMap of `AcidTxEntry`) tracks transaction state machine (Active/Committed/RolledBack/Failed) per transaction ID; `sql_transaction` handler now wires ACID tracking — detects BEGIN/COMMIT/ROLLBACK `SqlStatementKind` variants, creates registry entry on BEGIN, records each statement with affected table extraction, commits/rolls back on terminal statement. New endpoint `GET /api/v1/sql/transactions/active` returns live active transaction list via `sql_transactions_active` handler. AppState gains `acid_transactions: Arc<Mutex<AcidTransactionRegistry>>` field. Tests: `ws23_acid_tx_begin_commit_tracked_in_registry`, `ws23_acid_tx_rollback_tracked_in_registry` — both pass. **Next:** savepoint support, isolation level enforcement, conflict detection. |
...
**Not-started requirements (5 total):** REQ-08 (local+cloud SaaS), REQ-10 (trillion-row scale), REQ-19 (blazing ingest/update/read), REQ-21 (any-number-user concurrency), REQ-27 (native cache engine). REQ-07 has chunked-loader scaffold + KPI tests (async fan-out remains).
