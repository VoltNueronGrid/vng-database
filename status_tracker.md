...
| H-03 | Control-plane resilience hardening | Distributed Systems | P0 | R2 | In Progress | Control-plane clustering requirement and SPOF closure criteria documented; local chaos certification now covers degraded failover status under unresolved critical signals plus a multi-node targeted handoff churn pack via `tests/kpi/results/ws6/ws6-control-plane-chaos-smoke.json` and `tests/kpi/results/ws6/ws6-multi-node-cluster-chaos-smoke.json`, surfaced in the WS6 chaos matrix, release readiness, and closure artifacts (`tests/kpi/results/ws6/ws6-chaos-fault-matrix.json`, `tests/kpi/results/gates/ws6-release-readiness.json`, `tests/kpi/results/ws6/ws6-closure-gate-summary.json`); broader clustered runtime certification still pending |
| H-04 | Event durability hardening (outbox/replay) | Distributed Systems + SRE | P0 | R2 | In Progress | First local outbox/replay evidence pack now validates WAL recovery, checkpoint boundary preservation, replay-after-restore integrity, gap detection, and replay-only-unapplied semantics via `tests/kpi/results/h04/h04-outbox-replay-evidence.json`; broader event bus/outbox implementation still pending |
...
| H-03 | In Progress | 55% | improving | P0 | R2 | Extended local H-03 evidence from single-process leader churn into a multi-node targeted handoff churn pack, validated through `tests/kpi/results/ws6/ws6-control-plane-chaos-smoke.json`, `tests/kpi/results/ws6/ws6-multi-node-cluster-chaos-smoke.json`, `tests/kpi/results/ws6/ws6-chaos-fault-matrix.json`, `tests/kpi/results/gates/ws6-release-readiness.json`, and `tests/kpi/results/ws6/ws6-closure-gate-summary.json` | Full clustered runtime implementation and broader externalized multi-node chaos certification still pending | Push H-03 from in-memory multi-node replay into process-isolated cluster-runtime chaos certification |
| H-04 | In Progress | 38% | improving | P0 | R2 | Added first local outbox/replay evidence pack in `tests/kpi/results/h04/h04-outbox-replay-evidence.json`, covering WAL recovery, checkpoint boundary enforcement, replay-after-restore integrity, fault-injected gap detection, and replay-only-unapplied semantics | Event bus/outbox services pending | Promote the local H-04 pack into a formal gate once the event bus/outbox runtime exists |
