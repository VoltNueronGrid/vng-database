...
| REQ-27 | Native cache engine (Redis-like compat) | Epic 3, Epic 14 | In Progress | **Redis-compat cache endpoint (2026-04-08):** `POST /api/v1/cache/redis/command`; GET/SET/DEL/EXISTS/KEYS/FLUSH/PING; 4 ws27 tests. **EXPIRE/INCR/DECR (2026-04-09):** EXPIRE, INCR, INCRBY, DECR, DECRBY; 2 more ws27 tests. **Extended Redis commands (2026-04-10):** added MGET (multi-key get → JSON array), MSET (multi-key set from JSON object), GETSET (atomic get-then-set), LPUSH (prepend to JSON-array list), RPUSH (append to list), LLEN (list length), LRANGE (sub-range with Redis negative-index semantics); `RedisCacheCommandRequest` gains `keys: Option<Vec<String>>`, `start: Option<i64>`, `stop: Option<i64>` fields; list operations store lists as JSON arrays in existing `DistributedCacheManager`; 3 new tests: `ws27_redis_compat_mget_mset_lifecycle`, `ws27_redis_compat_getset_returns_old_sets_new`, `ws27_redis_compat_list_lpush_rpush_lrange_llen` — all 9 ws27 tests pass. **Next:** RESP3 wire protocol adapter, persistence layer, cluster fan-out. |
...
| WS2 | Epic 2 | Durability/storage/index/constraints | Storage Team | In Progress | WS0 (durability bootstrap + checkpoint/restart + disk-backed WAL adapter + WAL recovery wiring merged; index/constraint runtime endpoints now preserve operator-managed writes while exposing tenant-scoped read-only metadata access for namespaced indexes/constraints plus tenant-admin-managed create/add/drop paths through shared runtime RBAC, validated by `cargo test -p voltnuerongridd store_`, refreshed source-contract smoke `tests/kpi/results/ws2/ws2-index-constraint-smoke.json`, and live runtime smoke `tests/kpi/results/ws2/ws2-tenant-store-runtime-smoke.json`; gate orchestrator `run-ws2-gate.ps1` -> `tests/kpi/results/ws2/ws2-gate-summary.json`; workflow wiring in `.github/workflows/ci.yml`) |
...
| WS2 gate | passed | ready_for_validation | All packs passing. |
...
**Not-started requirements (0 total):** All requirements have active implementation. REQ-10/19 benchmark smoke script + artifact (2026-04-10). REQ-21: 6 ws21 tests as of 2026-04-10. REQ-23: 7 ws23 tests as of 2026-04-10. REQ-27: 9 ws27 tests as of 2026-04-10. REQ-07: async Tokio fan-out wired as of 2026-04-10. Total cargo test count: **144 passing**.
