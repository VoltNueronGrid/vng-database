param(
  [string]$CurrentSummaryPath = "tests/kpi/results/ws6/ws6-gate-summary.json",
  [string]$PriorSummaryPath = "tests/kpi/results/ws6/ws6-gate-summary.previous.json",
  [string]$OutputPath = "tests/kpi/results/ws6/ws6-gate-trend-comparison.json"
)
...
if (!(Test-Path -Path $CurrentSummaryPath)) {
  throw "Current WS6 summary not found at $CurrentSummaryPath"
}
...
if (!(Test-Path -Path $PriorSummaryPath)) {
  $baseline = [ordered]@{
    report = "ws6-gate-trend-comparison"
    status = "passed"
  ...
  Write-Host "WS6 trend comparison artifact: $OutputPath (baseline_established)"
  exit 0
}
...
foreach ($name in $allPackNames) {
  ...
  if ($before -ne $after) {
    $changes += [ordered]@{ pack = $name; before = $before; after = $after }
  }
  if ($before -eq "passed" -and $after -eq "failed") {
    $regressions += $name
  } elseif (($before -eq "failed" -or $before -eq "missing") -and $after -eq "passed") {
    $improvements += $name
  }
}
...
$artifact = [ordered]@{
  ...
  prior_summary = $PriorSummaryPath
  gate_status = [ordered]@{
    before = [string]$prior.status
    after = [string]$current.status
  }
  duration_ms = [ordered]@{
    before = [int]$prior.duration_ms
    after = [int]$current.duration_ms
    delta = $durationDelta
  }
...
Write-Host "WS6 trend comparison artifact: $OutputPath ($trendState)"
