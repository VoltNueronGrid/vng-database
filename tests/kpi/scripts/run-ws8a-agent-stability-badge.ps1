param(
  ...
  [string]$OutputPath = "tests/kpi/results/ws8a/ws8a-agent-stability-badge.json"
)
...
if (!(Test-Path -Path $SummaryPath)) { throw "WS8A summary not found at $SummaryPath" }
if (!(Test-Path -Path $TrendPath)) { throw "WS8A trend comparison not found at $TrendPath" }
...
Write-Host "WS8A agent stability badge artifact: $OutputPath ($message)"
