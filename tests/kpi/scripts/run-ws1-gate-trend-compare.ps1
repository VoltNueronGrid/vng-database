param(
  [string]$CurrentSummaryPath = "tests/kpi/results/ws1/ws1-gate-summary.json",
  [string]$PriorSummaryPath = "tests/kpi/results/ws1/ws1-gate-summary.previous.json",
  [string]$OutputPath = "tests/kpi/results/ws1/ws1-gate-trend-comparison.json"
)
...
if (!(Test-Path -Path $CurrentSummaryPath)) { throw "Current WS1 summary not found at $CurrentSummaryPath" }
$current = Get-Content -Raw -Path $CurrentSummaryPath | ConvertFrom-Json

if (!(Test-Path -Path $PriorSummaryPath)) {
  $baseline = [ordered]@{
    report = "ws1-gate-trend-comparison"
    status = "passed"
  ...
  Write-Host "WS1 trend comparison artifact: $OutputPath (baseline_established)"
  exit 0
}
...
Write-Host "WS1 trend comparison artifact: $OutputPath ($trendState)"
