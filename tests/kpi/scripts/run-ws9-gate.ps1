param(
  [string]$OutputPath = "tests/kpi/results/ws9/ws9-gate-summary.json"
)
...
$packs = @(
  @{
    Name = "ws9-studio-smoke"
    Script = "tests/kpi/scripts/run-ws9-studio-smoke.ps1"
    Artifact = "tests/kpi/results/ws9/studio-smoke.json"
  }
)
...
Write-Host "WS9 gate summary: $OutputPath ($status)"
if ($status -ne "passed") {
  exit 1
}
