param(
  ...
  [string]$OutputPath = "tests/kpi/results/ws22/ws22-pessimistic-lock-stability-badge.json"
)
...
if (!(Test-Path -Path $SummaryPath)) { throw "WS22 summary not found at $SummaryPath" }
if (!(Test-Path -Path $TrendPath)) { throw "WS22 trend comparison not found at $TrendPath" }
...
Write-Host "WS22 lock stability badge artifact: $OutputPath ($message)"
