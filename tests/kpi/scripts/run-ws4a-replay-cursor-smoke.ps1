param(
  [string]$OutputPath = "tests/kpi/results/ws4a/replay-cursor-smoke.json"
)
...
$start = Get-Date
$command = "cargo test -p voltnuerongrid-ingest replays_from_persisted_cursor_checkpoint -- --nocapture"
$outputLines = @()
$exitCode = 1

try {
  $testRun = Invoke-CargoTestCapture -Arguments @("test", "-p", "voltnuerongrid-ingest", "replays_from_persisted_cursor_checkpoint", "--", "--nocapture")
...
$artifact = [ordered]@{
  smoke = "ws4a-replay-cursor-bridge"
  status = $status
  command = $command
...
if ($status -ne "passed") {
  Write-Error "WS4A replay cursor smoke failed."
  exit 1
}

Write-Host "WS4A replay cursor smoke passed. Artifact: $OutputPath"
