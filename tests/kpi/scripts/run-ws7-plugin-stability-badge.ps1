param(
  ...
  [string]$OutputPath = "tests/kpi/results/ws7/ws7-plugin-stability-badge.json"
)
...
if (!(Test-Path -Path $SummaryPath)) { throw "WS7 summary not found at $SummaryPath" }
if (!(Test-Path -Path $TrendPath)) { throw "WS7 trend comparison not found at $TrendPath" }
...
$color = "yellow"
if ($gateStatus -eq "passed" -and ($trendState -eq "stable" -or $trendState -eq "improved" -or $trendState -eq "baseline_established")) {
  $color = "green"
...
Write-Host "WS7 plugin stability badge artifact: $OutputPath ($message)"
