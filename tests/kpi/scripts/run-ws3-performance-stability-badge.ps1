param(
  ...
  [string]$OutputPath = "tests/kpi/results/ws3/ws3-performance-stability-badge.json"
)
...
if (!(Test-Path -Path $SummaryPath)) { throw "WS3 summary not found at $SummaryPath" }
if (!(Test-Path -Path $TrendPath)) { throw "WS3 trend comparison not found at $TrendPath" }
...
$badge = [ordered]@{
  label = "ws3-performance-stability"
  message = $message
  color = $color
...
Write-Host "WS3 performance stability badge artifact: $OutputPath ($message)"
