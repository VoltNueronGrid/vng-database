param(
  [string]$CurrentSummaryPath = "tests/kpi/results/ws3/ws3-gate-summary.json",
  [string]$PriorSummaryPath = "tests/kpi/results/ws3/ws3-gate-summary.previous.json",
  [string]$OutputPath = "tests/kpi/results/ws3/ws3-gate-trend-comparison.json"
)
...
if (!(Test-Path -Path $CurrentSummaryPath)) { throw "Current WS3 summary not found at $CurrentSummaryPath" }
$current = Get-Content -Raw -Path $CurrentSummaryPath | ConvertFrom-Json

if (!(Test-Path -Path $PriorSummaryPath)) {
  $baseline = [ordered]@{
    report = "ws3-gate-trend-comparison"
    status = "passed"
  ...
  Write-Host "WS3 trend comparison artifact: $OutputPath (baseline_established)"
  exit 0
}
...
Write-Host "WS3 trend comparison artifact: $OutputPath ($trendState)"
