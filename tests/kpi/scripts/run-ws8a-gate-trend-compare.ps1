param(
  [string]$CurrentSummaryPath = "tests/kpi/results/ws8a/ws8a-gate-summary.json",
  [string]$PriorSummaryPath = "tests/kpi/results/ws8a/ws8a-gate-summary.previous.json",
  [string]$OutputPath = "tests/kpi/results/ws8a/ws8a-gate-trend-comparison.json"
)
...
if (!(Test-Path -Path $CurrentSummaryPath)) { throw "Current WS8A summary not found at $CurrentSummaryPath" }
$current = Get-Content -Raw -Path $CurrentSummaryPath | ConvertFrom-Json

if (!(Test-Path -Path $PriorSummaryPath)) {
  $baseline = [ordered]@{
    report = "ws8a-gate-trend-comparison"
    status = "passed"
  ...
  Write-Host "WS8A trend comparison artifact: $OutputPath (baseline_established)"
  exit 0
}
...
Write-Host "WS8A trend comparison artifact: $OutputPath ($trendState)"
