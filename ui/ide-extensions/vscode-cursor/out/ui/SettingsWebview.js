...
class SettingsPanel {
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel("vngSettings", "VoltNueronGrid Settings", vscode.ViewColumn.Beside, {
            enableScripts: true,
        ...
        return {
            editor: {
                fontSize: vscode.workspace
                    ...
                    .get("fontSize") || 14,
                fontFamily: vscode.workspace
                    .getConfiguration("editor")
                    .get("fontFamily") || "monospace",
                wordWrap: vscode.workspace
                    .getConfiguration("editor")
                    .get("wordWrap") || false,
            },
            sql: {
                runOnSave: config.get("sql.runOnSave") ||
                    "prompt",
                runOnOpen: config.get("sql.runOnOpen") ||
                    "never",
                timeout: config.get("sql.timeout") || 30,
                lineLength: config.get("sql.lineLength") || 80,
            },
            results: {
                rowsPerPage: config.get("results.rowsPerPage") || 25,
                autoRefresh: config.get("results.autoRefresh") || false,
                autoRefreshIntervalSeconds: config.get("results.autoRefreshIntervalSeconds") || 5,
                exportFormat: config.get("results.exportFormat") || "csv",
            },
            connection: {
                cacheSchemaEnabled: config.get("schema.cache.enabled") !== false,
                cacheTTLSeconds: config.get("schema.cache.ttlSeconds") || 300,
                retryAttempts: config.get("connection.retry.attempts") || 3,
                retryDelayMs: config.get("connection.retry.delayMs") || 500,
            },
        };
    }
