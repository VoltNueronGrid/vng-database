...
function registerSqlEditorFeatures(deps) {
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider([{ language: "sql" }, { pattern: "**/*.sql" }], {
    ...
    const saveHook = vscode.workspace.onDidSaveTextDocument(async (document) => {
        ...
        const runOnSave = vscode.workspace
            ...
            .get("sql.runOnSave", "prompt");
        if (runOnSave === "never") {
            return;
        }
        if (runOnSave === "prompt") {
            const choice = await vscode.window.showInformationMessage(`Run SQL from ${document.fileName.split(/[\\/]/).pop()}?`, "Run", "Skip");
            if (choice !== "Run") {
                return;
            }
        }
    ...
    const openHook = vscode.workspace.onDidOpenTextDocument(async (document) => {
        ...
        const runOnOpen = vscode.workspace
            .getConfiguration("voltnuerongrid")
            .get("sql.runOnOpen", "never");
        if (runOnOpen === "never") {
            return;
        }
        if (runOnOpen === "prompt") {
            const choice = await vscode.window.showInformationMessage(`Run SQL from ${document.fileName.split(/[\\/]/).pop()} on open?`, "Run", "Skip");
            if (choice !== "Run") {
                return;
            }
        }
    ...
    const diagnosticsProvider = vscode.languages.registerCodeActionsProvider([{ language: "sql" }, { pattern: "**/*.sql" }], {
        provideCodeActions(document, range, context) {
            ...
            for (const diagnostic of context.diagnostics) {
                ...
                const targetRange = range.isEqual(diagnostic.range) || range.contains(diagnostic.range.start)
                    ? diagnostic.range
                    : diagnostic.range;
                for (const suggestion of data.suggestions.slice(0, 3)) {
                    const title = data.kind === "table"
    ...
    const updateSqlDiagnostics = async (document) => {
        if (!isSqlDocument(document)) {
...
function getSqlToRun(editor) {
    const selected = editor.document.getText(editor.selection);
    return selected.trim().length > 0 ? selected : editor.document.getText();
}
async function runSqlAndPresent(operation, sql, connection, output, analyze, explain = false) {
    const response = explain
        ? await (0, client_1.explainSql)(connection, sql)
        : analyze
            ? await (0, client_1.analyzeSql)(connection, sql)
            : await (0, client_1.executeSql)(connection, sql);
...
async function runManagedSqlAndPresent(operation, sql, connection, deps) {
    ...
    const results = await deps.queryExecutionService.executeStatementsStream(connection, statements, {
        ...
        onResult: async (result, index, total) => {
            deps.output.appendLine(`[${operation}] statement ${index}/${total} status=${result.status} rows=${result.rowCount} time=${result.executionTime}ms`);
...
async function resolveManagedConnection(deps) {
...
async function computeDiagnostics(document, schemaManager, connection) {
    const diagnostics = [];
...
function addTableCompletionItems(items, tables, activeClause) {
    const seen = new Set();
...
function addColumnCompletionItems(items, tables, activeClause, aliasTarget) {
    const seen = new Set();
    for (const table of tables) {
...
function clearDiagnosticDataForDocument(uri, map) {
