// Users & Roles panel — client-side mock until server endpoints exist.
...
function load(): UserDraft[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) return JSON.parse(raw) as UserDraft[];
  } catch {}
  // Seed with a couple of plausible users so the UI isn't empty.
  return [
...
export function UsersPanel() {
  const [users, setUsers] = useState<UserDraft[]>(() => load());
    ...
    <div>
      ...
      {users.map((u) => (
        ...
        >
          ...
          <span className="conn-type-badge" style={{ background: roleBg(u.role), color: roleFg(u.role), borderColor: roleBd(u.role) }}>
            {u.role}
          </span>
        </div>
      ))}
      ...
      <div style={{ padding: "12px", fontSize: 10.5, color: "var(--text-3)", lineHeight: 1.5 }}>
