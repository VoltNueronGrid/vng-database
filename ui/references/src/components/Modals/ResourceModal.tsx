...
function renderBody(
  ctx: NonNullable<ReturnType<typeof useModalStore.getState>["current"]>,
  runSql: (sql: string, fileName: string) => void
) {
  switch (ctx.kind) {
...
function Footer({ onCancel, onSubmit, label = "Generate SQL", danger = false }: {
  onCancel: () => void; onSubmit: () => void; label?: string; danger?: boolean;
}) {
  return (
...
function CreateDatabaseForm({ onSubmit }: { onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
...
function CreateSchemaForm({ db, onSubmit }: { db: string; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [name, setName] = useState("");
    ...
    <>
      <div className="form-row">
        ...
        <Field label="Owner (optional)">
          <input className="form-input" value={owner} onChange={(e) => setOwner(e.target.value)} placeholder="user or role" />
        </Field>
      </div>
      <Footer
        onCancel={close}
        onSubmit={() => {
          if (!name) return;
          const owners = owner ? `\n  AUTHORIZATION ${owner}` : "";
          onSubmit(`CREATE SCHEMA ${db}.${name}${owners};`, `create_schema_${name}.sql`);
        }}
      />
    </>
  );
...
function CreateTableForm({ target, onSubmit }: { target: string; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [name, setName] = useState("");
  ...
  function addCol() {
    setCols((cs) => [...cs, { name: "", type: "VARCHAR(255)", pk: false, nullable: true, defaultValue: "" }]);
  }
  ...
  return (
    <>
      ...
      <div className="col-list" style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", padding: 6, maxHeight: 280, overflowY: "auto" }}>
        ...
        {cols.map((c, i) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "1.5fr 1.5fr 50px 50px 1.2fr 28px", gap: 6, padding: "3px 4px", alignItems: "center" }}>
            ...
            <input className="form-input" style={{ height: 26, fontSize: 11.5 }} value={c.defaultValue} onChange={(e) => update(i, { defaultValue: e.target.value })} placeholder="—" />
            <button className="btn btn-sm" style={{ padding: "0 6px" }} onClick={() => removeCol(i)} title="Remove">✕</button>
          </div>
        ))}
      </div>
      <button className="btn btn-sm" style={{ alignSelf: "flex-start", marginTop: 6 }} onClick={addCol}>+ Add Column</button>
...
function DropForm({ what, target, onSubmit }: { what: string; target: string; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [confirm, setConfirm] = useState("");
  const [cascade, setCascade] = useState(false);
  const matches = confirm === target.split(".").pop();
  return (
    <>
      ...
      <Field label={`Type "${target.split(".").pop()}" to confirm`} full>
      ...
      <Footer
        ...
        onSubmit={() => {
          ...
          onSubmit(`DROP ${what} IF EXISTS ${target}${c};`, `drop_${target}.sql`);
        }}
      />
    </>
  );
...
function TruncateForm({ target, onSubmit }: { target: string; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [confirm, setConfirm] = useState("");
  ...
  return (
    <>
      ...
      <Footer
        ...
        onSubmit={() => {
          if (!matches) return;
          const r = restart ? " RESTART IDENTITY" : "";
          onSubmit(`TRUNCATE TABLE ${target}${r};`, `truncate_${target}.sql`);
        }}
      />
    </>
  );
...
function RenameForm({ kind, target, onSubmit }: { kind: string; target: string; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  ...
  return (
    <>
      <Field label="New Name" full>
        <input className="form-input" value={next} onChange={(e) => setNext(e.target.value)} placeholder={target.split(".").pop()} autoFocus />
      </Field>
      <Footer
        onCancel={close}
        onSubmit={() => {
...
function EditColumnForm({ target, col, onSubmit }: { target: string; col: SchemaColumn; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  ...
  return (
    <>
      <div className="form-row">
        ...
        <Field label="Type">
          <select className="form-select" value={type} onChange={(e) => setType(e.target.value)}>
            {[type, ...TYPE_CHOICES].filter((v, i, a) => a.indexOf(v) === i).map((t) => <option key={t}>{t}</option>)}
          </select>
        </Field>
      </div>
      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, marginTop: 8, color: "var(--text-2)" }}>
      ...
      <Footer
        ...
        onSubmit={() => {
          const stmts: string[] = [];
          if (newName !== col.name) stmts.push(`ALTER TABLE ${tableTarget} RENAME COLUMN ${col.name} TO ${newName};`);
          if (type !== col.data_type) stmts.push(`ALTER TABLE ${tableTarget} ALTER COLUMN ${newName} TYPE ${type};`);
          if (nullable !== col.nullable) stmts.push(`ALTER TABLE ${tableTarget} ALTER COLUMN ${newName} ${nullable ? "DROP" : "SET"} NOT NULL;`);
...
function DropColumnForm({ target, col, onSubmit }: { target: string; col: SchemaColumn; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const tableTarget = target.split(".").slice(0, 3).join(".");
  return (
    <>
      ...
      <Footer
        ...
        danger
        label="Drop Column"
        onSubmit={() => onSubmit(`ALTER TABLE ${tableTarget} DROP COLUMN ${col.name};`, `drop_col_${col.name}.sql`)}
      />
    </>
  );
}

function CreateUserForm({ onSubmit }: { onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [name, setName] = useState("");
...
function CreateRoleForm({ onSubmit }: { onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [name, setName] = useState("");
  return (
    <>
      <Field label="Role Name" full>
        <input className="form-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. analyst_role" autoFocus />
      </Field>
      <Footer onCancel={close} onSubmit={() => name && onSubmit(`CREATE ROLE ${name};`, `create_role_${name}.sql`)} />
    </>
  );
}

function GrantRoleForm({ user, onSubmit }: { user: string; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const [role, setRole] = useState("readonly");
  return (
    <>
      ...
      <Footer onCancel={close} onSubmit={() => onSubmit(`GRANT ${role} TO ${user};`, `grant_${user}.sql`)} />
    </>
  );
}

function ViewDdlForm({ target, payload, onSubmit }: { target: string; payload?: Record<string, unknown>; onSubmit: (sql: string, n: string) => void }) {
  const close = useModalStore((s) => s.close);
  const ddl = useMemo(() => {
    ...
    if (kind === "table" && payload?.table) {
      ...
      const lines = t.columns.map((c) => {
        ...
        return `  ${c.name} ${c.data_type}${nn}${pk}`;
      });
