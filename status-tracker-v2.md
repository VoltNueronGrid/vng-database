...
| S2-WS2-02 | WS2 | WAL + checkpoint **in-memory** + file adapter patterns | **PARTIAL** | Not full recovery at scale. **Session 9:** WAL COMMIT path now appends every INSERT/UPDATE/DELETE mutation to `InMemoryDurabilityEngine` via `wal.append_mutation()` + `wal.maybe_checkpoint()` (called after each `sql_transaction` COMMIT); `wal_records()` + `checkpoint_count()` accessor methods added to `InMemoryDurabilityEngine` in `crates/voltnuerongrid-store/src/lib.rs`; `wal_engine: Arc<Mutex<InMemoryDurabilityEngine>>` field added to AppState; `GET /api/v1/store/wal/status` → `WalStatusResponse {status, wal_len, latest_sequence, checkpoint_count}`; `POST /api/v1/store/wal/recover` → replays WAL records into PagedRowStore (full or `dry_run=true`); 3 integration tests `s2_ws2_02_*` verify zero-on-fresh-state, wal_len ≥ 2 after COMMIT, dry_run non-destructive. |
