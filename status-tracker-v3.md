...
|------|----------------------------------|
| **TLS on the native socket** | **Partial:** rustls accept path + `VNG_NATIVE_TLS_CERT_PATH` / `VNG_NATIVE_TLS_KEY_PATH` + handshake timeout in `run_native_listener`. **Follow-on:** mTLS, rotation, structured error mapping, integration tests (NT-S6). |
| **Connection limits / idle timeouts** | **Partial:** `max_connections` semaphore (reject + shutdown) + per-read idle timeout on framed reads. **Follow-on:** metrics, heartbeat alignment, soak tests. |
| **True auto fallback (native fails → HTTP) without a second URL** | One URL cannot be both `vng://` and `http://`. Fallback implies two endpoints (or an explicit discovery rule for the HTTP port), retry policy, and diagnostics (`fallbackTriggered`, reason). **Addressed in part** by dual URL (`http_fallback_url` / `httpFallbackUrl`) + `resolve_auto_transport` / `resolveAutoTransport`, and by **port-based discovery** (`infer_http_base_url_from_vng_url` + `resolve_auto_transport_with_discovery`) when the caller supplies the HTTP port — still not “derive everything from one URL string” without that parameter or injected capabilities. |
...
| ID | Theme | Notes |
|---|---|---|
| NT-S5-001..002 | Default `auto` in IDE + observability UI | `voltnuerongrid.transportMode` default now `auto` (extension); observability UI + TS native data-plane still follow-on |
| NT-S6-001 | TLS/mTLS + hardened auth on native listener | TLS accept wired (rustls + env cert paths); mTLS/hardened auth/tests still follow-on |
