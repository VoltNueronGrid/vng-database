...
- 2026-04-18: Structured stderr logs for the native listener (`component`=`vng_native_listener`, `event` per lifecycle: `listener_config`, `accepted`, `connection_rejected`, `read_idle_timeout`, TLS events, etc.). Drivers: `VNG_HTTP_DISCOVERY_PORT` merged into `resolve_auto_transport_with_discovery` / `infer_transport_capabilities_tcp_with_discovery` when explicit port is unset; conformance case `tm-auto-discovery-implicit-dual`; TS/Python TCP probe helpers aligned with Rust (`probeTcpConnect`, `inferTransportCapabilitiesTcp`, …).
- 2026-04-18: Native COMMAND `ingest.schema.registry` (payload.command string) returns the same registry snapshot as HTTP `GET /api/v1/ingest/schema/registry` (shared `collect_ingest_schema_registry_response`). Optional **mTLS**: `VNG_NATIVE_TLS_CLIENT_CA_PATH` points to PEM trust anchors for client certificates when TLS is enabled. TS `nativeSession.ts` / Python `native_session.py` add Hello/Auth/health roundtrip helpers; VS Code **VoltNueronGrid Transport** output channel logs transport context for queries.
