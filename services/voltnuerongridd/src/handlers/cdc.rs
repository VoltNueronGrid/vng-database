...
pub(crate) async fn cdc_stream(
    ...
    let wal = state.storage.wal_engine.lock().expect("wal_engine lock cdc");
...
pub(crate) async fn cdc_stream_filter(
    ...
    let wal = state.storage.wal_engine.lock().expect("wal_engine lock cdc_filter");
...
pub(crate) async fn cdc_stream_latest(
    ...
    let wal = state.storage.wal_engine.lock().expect("wal_engine lock cdc_latest");
    let all_events: Vec<CdcEvent> = wal.wal_records()
        .iter()
        .map(|r| CdcEvent {
            sequence: r.sequence,
...
pub(crate) async fn cdc_cursor_status(
    ...
    let cursors = state.storage.cdc_cursors.lock().expect("cdc_cursors lock");
...
pub(crate) async fn cdc_cursor_advance(
    ...
    let mut cursors = state.storage.cdc_cursors.lock().expect("cdc_cursors lock");
...
pub(crate) async fn cdc_metrics(
    ...
    require_operator_auth(&headers, &state)?;
    let wal = state.storage.wal_engine.lock().expect("wal_engine cdc_metrics lock");
