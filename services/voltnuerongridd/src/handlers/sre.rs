...
pub(crate) async fn sre_incident_diagnose(
    ...
    require_operator_auth(&headers, &state)?;
    let failure_type = req.failure_type.as_deref().unwrap_or("unknown");
    let severity = req.severity.as_deref().unwrap_or("low");
    let message = req.message.as_deref().unwrap_or("");

    // Shared classifier (A-8 reuse): configurable rules first, then built-in patterns.
    let (root_cause, confidence, recommended_action) =
        crate::helpers::autonomous_exec::classify_incident(&state, failure_type, severity, message);
